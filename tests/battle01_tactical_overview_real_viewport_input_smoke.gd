...
func _run() -> void:
	...
	var presentation: Battle3DPresentation = battle.get_node_or_null("World3D/Presentation3D") as Battle3DPresentation
	...
	var staged_before: Dictionary = _staged_snapshot(staging, [recon, infantry, ifv, supply])
	var intents_before: Dictionary = _intent_snapshot(staging, [recon, infantry, ifv, supply])
	...
	var drag_from_screen: Vector2 = map_rect.position + drag_from_local
	var drag_to_screen: Vector2 = map_rect.position + drag_to_local
	var expected_drag: Vector2 = _minimap_local_to_sim(minimap, drag_to_local)
	var focus_before_drag: Vector2 = camera.focus_sim
	await _mouse_drag(drag_from_screen, drag_to_screen)
	var drag_ok: bool = camera.focus_sim.distance_to(expected_drag) <= 3.0 and camera.focus_sim.distance_to(focus_before_drag) > 20.0
	_require(drag_ok, "REAL_VIEWPORT_MINIMAP_DRAG_CAMERA3D_PASS")
	...
	var isolation_ok: bool = (
		...
		and _snapshot_vectors_equal(_staged_snapshot(staging, [recon, infantry, ifv, supply]), staged_before)
		and _intent_snapshots_equal(_intent_snapshot(staging, [recon, infantry, ifv, supply]), intents_before)
	)
	_require(isolation_ok, "REAL_VIEWPORT_MINIMAP_WORLD_INPUT_ISOLATION_PASS")

	var nav_does_not_start: bool = (
	...
	_require(nav_does_not_start, "REAL_VIEWPORT_MINIMAP_NAV_DOES_NOT_START_BATTLE_PASS")

	# Navigate to the hidden RED side through the real minimap event chain. Camera
	# movement must never grant intelligence or reveal 3D/minimap RED truth.
	...
	# Camera keyboard pan remains independent from pointer routing.
	...
	# World wheel still reaches Battle3DInput when the pointer is not over GUI.
	...
	# Real viewport formation placement drag: press the real 3D marker, move across
	# the world, then release at another contract-valid staging point.
	...
	var placement_ok: bool = staging.get_staged_position_for(ifv).distance_to(placement_target) <= 4.0 and ifv.global_position.distance_to(placement_target) <= 4.0
	_require(placement_ok, "REAL_VIEWPORT_FORMATION_PLACEMENT_DRAG_PASS")

	# Real viewport formation click selection remains available in world space.
	...
	# Empty-ground LMB drag remains box select rather than placement.
	selection.clear_selection()
	var box_center: Vector2 = ifv_screen
	var box_from := box_center + Vector2(-78.0, -68.0)
	var box_to := box_center + Vector2(78.0, 68.0)
	await _mouse_drag(box_from, box_to)
	_require(ifv in selection.get_selected() and not input3d.is_world_pointer_interaction_active_for_test(), "REAL_VIEWPORT_EMPTY_GROUND_BOX_SELECT_PASS")

	# RMB and Shift+RMB still enter the existing staging queue through the real
	# Viewport path. They do not execute movement before START.
	camera.focus_on_sim(Vector2(900.0, 900.0))
	await process_frame
	selection.select_only(recon)
	...
	var move_ok: bool = staging.get_initial_intent_for(recon) == BattlePreBattleStagingController.INTENT_MOVE and recon.global_position.is_equal_approx(recon_before) and recon.get_order() == "HOLD"
	_require(move_ok, "REAL_VIEWPORT_STAGING_RMB_INITIAL_MOVE_PASS")
	...
	var advance_ok: bool = staging.get_initial_intent_for(ifv) == BattlePreBattleStagingController.INTENT_ADVANCE and ifv.global_position.is_equal_approx(ifv_before) and ifv.get_order() == "HOLD"
	_require(advance_ok, "REAL_VIEWPORT_STAGING_SHIFT_RMB_INITIAL_ADVANCE_PASS")

	# A real staging HUD button click must be GUI-owned. It clears the selected
	# IFV's queued ADVANCE but does not enter the world input handler.
	...
	# START is also injected through the real Viewport/Button path. After T=0 the
	# exact same minimap Control continues driving the same BattleCamera3D.
	...
	var live_ok: bool = not staging.is_staging_active() and camera.focus_sim.distance_to(live_expected) <= 3.0 and camera.focus_sim.distance_to(live_before) > 20.0
	_require(live_ok, "REAL_VIEWPORT_LIVE_MINIMAP_CAMERA3D_PASS")
...
func _mouse_click(screen_position: Vector2, button_index: MouseButton, shift_pressed: bool) -> void:
...
func _mouse_drag(from_screen: Vector2, to_screen: Vector2) -> void:
	...
	Input.parse_input_event(press)
	await process_frame
	var mid := from_screen.lerp(to_screen, 0.5)
	for point: Vector2 in [mid, to_screen]:
		var motion := InputEventMouseMotion.new()
		motion.position = point
		motion.relative = point - from_screen
...
func _mouse_wheel(screen_position: Vector2, wheel_button: MouseButton) -> void:
...
func _key_hold(keycode: Key, frames: int) -> void:
...
func _staged_snapshot(staging: BattlePreBattleStagingController, formations: Array[BattleFormation]) -> Dictionary:
	var result: Dictionary = {}
	for formation: BattleFormation in formations:
		result[formation.display_name] = staging.get_staged_position_for(formation)
	return result

func _intent_snapshot(staging: BattlePreBattleStagingController, formations: Array[BattleFormation]) -> Dictionary:
	var result: Dictionary = {}
	for formation: BattleFormation in formations:
...
func _snapshot_vectors_equal(a: Dictionary, b: Dictionary) -> bool:
	...
	for key: Variant in a.keys():
		if not b.has(key) or not (Vector2(a[key]) as Vector2).is_equal_approx(Vector2(b[key])):
			return false
	return true
...
func _intent_snapshots_equal(a: Dictionary, b: Dictionary) -> bool:
	...
	for key: Variant in a.keys():
		...
		if str(left.get("kind", "")) != str(right.get("kind", "")):
			return false
		if not Vector2(left.get("target", Vector2.ZERO)).is_equal_approx(Vector2(right.get("target", Vector2.ZERO))):
			return false
	return true
...
func _visible_red_proxy_count(presentation: Battle3DPresentation) -> int:
	...
	for child: Node in presentation.get_children():
		if child is Node3D and child.name.begins_with("Proxy_RED") and (child as Node3D).visible:
			count += 1
	return count
