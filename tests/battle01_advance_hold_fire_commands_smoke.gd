...
func _scenario_real_advance_and_move_difference() -> void:
	...
	var infantry: BattleFormation = c["infantry"] as BattleFormation
	var target: BattleFormation = c["red_inf2"] as BattleFormation
	var selection: BattleSelectionController = c["selection"] as BattleSelectionController
	var input3d: Battle3DInput = c["input3d"] as Battle3DInput
	var intel: BattleIntelTracker = c["intel"] as BattleIntelTracker
	...
	var move_ammo: int = infantry.current_ammo
	var move_fire: int = infantry.get_fire_serial()
	var move_hp: int = target.current_hp
	...
	var move_plain: bool = infantry.current_ammo == move_ammo and infantry.get_fire_serial() == move_fire and target.current_hp == move_hp
	...
	infantry._fire_cooldown = 0.0
	infantry._update_combat()
	var legal_fire: bool = infantry.current_ammo == ammo_before - 1 and infantry.get_fire_serial() == fire_before + 1 and target.current_hp == hp_before - infantry.calculate_attack_damage(target)
	_require(move_plain and legal_fire, "MOVE_ADVANCE_BEHAVIOR_DIFFERENCE_PASS")
	_require(legal_fire, "ADVANCE_LEGAL_FIRE_PASS")

	var start: Vector2 = infantry.global_position
	_advance_movement_to_hold(infantry)
	_require(advance_started and start.distance_to(infantry.global_position) > 100.0 and infantry.get_order() == "HOLD" and infantry.global_position.distance_to(Vector2(900.0, 700.0)) <= 6.0, "ADVANCE_REAL_MOVEMENT_PASS")
...
func _scenario_confirmed_only() -> void:
	...
	var unseen_ok: bool = _assert_no_advance_fire(selection, infantry, target)
	_force_intel(intel, target, BattleIntelTracker.CONTACT)
	var contact_ok: bool = _assert_no_advance_fire(selection, infantry, target)
	_force_intel(intel, target, BattleIntelTracker.LAST_KNOWN)
	var last_known_ok: bool = _assert_no_advance_fire(selection, infantry, target)
	_force_intel(intel, target, BattleIntelTracker.CONFIRMED)
	selection.force_advance_decision_for_test()
	var ammo: int = infantry.current_ammo
	var fire: int = infantry.get_fire_serial()
	infantry._fire_cooldown = 0.0
	infantry._update_combat()
	var confirmed_ok: bool = infantry.current_ammo == ammo - 1 and infantry.get_fire_serial() == fire + 1
	_require(unseen_ok and contact_ok and last_known_ok and confirmed_ok, "ADVANCE_CONFIRMED_ONLY_PASS")
	await _dispose_battle(c)

func _scenario_los_range_target_loss() -> void:
	var c: Dictionary = await _spawn_battle()
	var ifv: BattleFormation = c["ifv"] as BattleFormation
	var target: BattleFormation = c["red_inf2"] as BattleFormation
	var selection: BattleSelectionController = c["selection"] as BattleSelectionController
	var intel: BattleIntelTracker = c["intel"] as BattleIntelTracker
	...
	_require(_assert_no_advance_fire(selection, ifv, target), "ADVANCE_LOS_GATE_PASS")
	...
	var stored: Vector2 = selection.get_advance_destination_for_test(ifv)
	...
	_require(no_fire and selection.is_advancing(ifv) and selection.get_advance_destination_for_test(ifv).is_equal_approx(stored) and ifv.global_position.x > before.x, "ADVANCE_NO_CHASE_OUT_OF_RANGE_PASS")
	...
	var fire_before: int = ifv.get_fire_serial()
	var move_before: Vector2 = ifv.global_position
	selection.force_advance_decision_for_test()
	ifv._fire_cooldown = 0.0
	ifv._update_combat()
	ifv._update_movement(0.10)
	_require(ifv.current_ammo == ammo_before and ifv.get_fire_serial() == fire_before and selection.is_advancing(ifv) and selection.get_advance_destination_for_test(ifv).is_equal_approx(stored) and ifv.global_position.distance_to(move_before) > 0.1, "ADVANCE_TARGET_LOSS_CONTINUES_PASS")
	await _dispose_battle(c)

func _scenario_deterministic_target() -> void:
	var c: Dictionary = await _spawn_battle()
	var infantry: BattleFormation = c["infantry"] as BattleFormation
	var red1: BattleFormation = c["red_inf1"] as BattleFormation
	var red2: BattleFormation = c["red_inf2"] as BattleFormation
	var selection: BattleSelectionController = c["selection"] as BattleSelectionController
	var intel: BattleIntelTracker = c["intel"] as BattleIntelTracker
	...
	infantry._fire_cooldown = 0.0
	infantry._update_combat()
	var tie_ok: bool = red1.current_hp < hp1 and red2.current_hp == hp2
	red1.current_hp = hp1
	red2.current_hp = hp2
	...
	infantry._fire_cooldown = 0.0
	infantry._update_combat()
	_require(tie_ok and red1.current_hp == hp1 and red2.current_hp < hp2, "ADVANCE_DETERMINISTIC_TARGET_PASS")
	await _dispose_battle(c)

func _scenario_hold_fire() -> void:
	var c: Dictionary = await _spawn_battle()
	var infantry: BattleFormation = c["infantry"] as BattleFormation
	var recon: BattleFormation = c["recon"] as BattleFormation
	var ifv: BattleFormation = c["ifv"] as BattleFormation
	var supply: BattleFormation = c["supply"] as BattleFormation
	var armor: BattleFormation = c["red_armor"] as BattleFormation
	var target: BattleFormation = c["red_inf2"] as BattleFormation
	var selection: BattleSelectionController = c["selection"] as BattleSelectionController
	var intel: BattleIntelTracker = c["intel"] as BattleIntelTracker
	...
	selection.handle_input(h_event, Vector2.ZERO)
	infantry.set_combat_target(target)
	var ammo_before: int = infantry.current_ammo
	var fire_before: int = infantry.get_fire_serial()
	var hp_before: int = target.current_hp
	for _i: int in range(4):
		infantry._fire_cooldown = 0.0
		infantry._update_combat()
	_require(infantry.is_hold_fire_enabled() and infantry.current_ammo == ammo_before and infantry.get_fire_serial() == fire_before and target.current_hp == hp_before, "HOLD_FIRE_SUPPRESSES_ATTACK_PASS")
	...
	selection.force_advance_decision_for_test()
	var held_ammo: int = infantry.current_ammo
	var held_fire: int = infantry.get_fire_serial()
	for _step: int in range(4):
		infantry._fire_cooldown = 0.0
		infantry._update_combat()
		infantry._update_movement(0.10)
	var held_advance: bool = selection.is_advancing(infantry) and infantry.current_ammo == held_ammo and infantry.get_fire_serial() == held_fire and infantry.is_hold_fire_enabled()
	...
	infantry._fire_cooldown = 0.0
	infantry._update_combat()
	_require(selection.is_advancing(infantry) and not infantry.is_hold_fire_enabled() and infantry.current_ammo == release_ammo - 1 and infantry.get_fire_serial() == release_fire + 1, "HOLD_FIRE_RELEASE_RESUMES_FIRE_PASS")

	var before_roles: Array = [[recon.can_capture, recon.can_contest], [infantry.can_capture, infantry.can_contest], [ifv.can_capture, ifv.can_contest], [armor.can_capture, armor.can_contest], [supply.can_capture, supply.can_contest]]
	...
	var after_roles: Array = [[recon.can_capture, recon.can_contest], [infantry.can_capture, infantry.can_contest], [ifv.can_capture, ifv.can_contest], [armor.can_capture, armor.can_contest], [supply.can_capture, supply.can_contest]]
	_require(before_roles == after_roles, "HOLD_FIRE_OBJECTIVE_RULES_UNCHANGED_PASS")
...
func _scenario_direct_overrides() -> void:
	...
	var infantry: BattleFormation = c["infantry"] as BattleFormation
	var selection: BattleSelectionController = c["selection"] as BattleSelectionController
	var flow: BattlePlayerWarFlow = c["flow"] as BattlePlayerWarFlow
	var resupply: BattleResupplyController = c["resupply"] as BattleResupplyController
	...
	_require(resupply_started and resupply.is_resupply_active() and not selection.is_advancing(infantry) and infantry.get_order() != "ADVANCE" and infantry.is_hold_fire_enabled(), "ADVANCE_RESUPPLY_OVERRIDE_PASS")
	await _dispose_battle(c)
...
func _scenario_formation_aware_and_logistics() -> void:
	...
	var supply: BattleFormation = c["supply"] as BattleFormation
	var selection: BattleSelectionController = c["selection"] as BattleSelectionController
	var navigation: BattleNavigation = c["navigation"] as BattleNavigation
	...
	_require(issued == 2 and navigation.path_uses_north_foot_link(recon_path) and not navigation.path_uses_north_foot_link(ifv_path), "ADVANCE_FORMATION_AWARE_MOBILITY_PASS")
	...
	_require(supply_only == 0 and supply.get_order() != "ADVANCE" and mixed == 1 and infantry.get_order() == "ADVANCE" and supply.get_order() != "ADVANCE", "LOGISTICS_ADVANCE_REJECTED_PASS")
	await _dispose_battle(c)
...
func _scenario_reserve_compatibility() -> void:
	...
	if reserve != null:
		...
		ok = advanced == 1 and held == 1 and selection.is_advancing(reserve) and reserve.is_hold_fire_enabled()
	_require(ok, "RESERVE_COMMAND_COMPATIBILITY_PASS")
	await _dispose_battle(c)
...
func _spawn_battle() -> Dictionary:
	...
	return {
		...
		"input3d": battle.get_node("Input3D") as Battle3DInput,
		...
		"red_armor": roster.enemy_armor[0] as BattleFormation,
	}
...
func _assert_no_advance_fire(selection: BattleSelectionController, formation: BattleFormation, target: BattleFormation) -> bool:
	selection.force_advance_decision_for_test()
	var ammo: int = formation.current_ammo
	var fire: int = formation.get_fire_serial()
	var hp: int = target.current_hp
	formation._fire_cooldown = 0.0
	formation._update_combat()
	return formation.current_ammo == ammo and formation.get_fire_serial() == fire and target.current_hp == hp
