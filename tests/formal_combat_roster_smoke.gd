...
func _run() -> void:
	...
	for unit: BattleFormation in infantry:
		...
		_check(unit.ammo_capacity == 24, "Infantry ammo=24")
		_check(is_equal_approx(unit.move_speed, 115.0), "Infantry speed=115")
		_check(unit.can_capture and unit.can_contest, "Infantry captures and contests")
	...
	_check(armor.ammo_capacity == 16, "Armor ammo=16")
	_check(is_equal_approx(armor.move_speed, 85.0), "Armor speed=85")
	_check(not armor.can_capture and armor.can_contest, "Armor contests but cannot capture")
	...
	_check(not truck.can_contest, "Supply Truck cannot contest")
	...
	_check(not reinforcement_inf.visible, "Reinforcement Infantry hidden until activation")
	_check(not reinforcement_arm.visible, "Reinforcement Armor hidden until activation")
