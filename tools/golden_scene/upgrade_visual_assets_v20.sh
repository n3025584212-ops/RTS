...
mapfile -t OBJS < <(find "$TMP/src" -type f -iname '*.obj' | sort)
mapfile -t BLENDS < <(find "$TMP/src" -type f -iname '*.blend' | sort)
...
elif [ "${#BLENDS[@]}" -ge 1 ]; then
  blender -b "${BLENDS[0]}" --python "$SPLITTER" -- --out "$DEST" --max 6
else
  echo "V20 house collection contains no supported OBJ/Blend source" >&2
  exit 20
fi
...
cat > "$OUT/.visual_assets_v20_complete" <<EOF
