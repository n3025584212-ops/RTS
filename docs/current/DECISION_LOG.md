...
## 2026-09-20 — the GPT four-window routing layer is retired; a single execution model replaces it

DATE=2026-09-20
DECISION_ID=FRONTLINE_EXECUTION_MODEL_V1_ADOPTED
STATUS=ACCEPTED
DECISION=The user directed that the project continue without the GPT four-window layer and that the executing agent define its own operating model. The 00/01/02/03 system is retired, together with the two governance documents that defined it and the `docs/gpt_windows/` initialization prompts; all three are archived under `docs/archive/governance/`. `docs/FRONTLINE_EXECUTION_MODEL_V1.md` becomes the active execution model: one state (`CURRENT_STATE.md`), one active task contract, a four-step read path, the hard evidence rules, and review independence enforced structurally rather than claimed. The reviewer is now an isolated sub-agent that receives only the task contract and the evidence directory and is told nothing about the builder's reasoning, so independence no longer rests on "a different conversation" or on a different label.
EVIDENCE=Rationale recorded in `docs/FRONTLINE_EXECUTION_MODEL_V1.md` section 1: every stale state declaration cleaned on 2026-09-20 (six files, 39 insertions / 32 deletions) lived in the collaboration layer rather than in a product document, so the window layer was producing handoff cost and drift without a verification gain. Enforcement continues through `tools/check_doc_authority.py`, whose read-path list now names the execution model document; after the change it reports clean over 39 documents and 40 read-path references.
IMPACT=Retired and archived: `GPT_FOUR_WINDOW_SYSTEM_V5.md`, `GPT_WINDOW_RUNTIME_PLAN_V2.md`, `gpt_windows/` (seven renames recorded). Rewritten references: CHARTER_V3 header flags plus sections 3 and 12, PROJECT_SYSTEM_V3, CURRENT_STATE (new `EXECUTION_MODEL` key; the `Four-window allocation` section deleted), START_HERE, docs/README, DOCUMENT_AUTHORITY_POLICY_V1, and the audit contract, which is de-windowed to ROLE=INDEPENDENT_REVIEW_AND_FALSIFICATION while its filename keeps the historical token for traceability. Product constraints, the gate chain and the open route decision are unchanged: `NEXT=USER_SELECTS` — human playtest checkpoint or Gate D3.
RELATED_ISSUE_OR_PR=#41
