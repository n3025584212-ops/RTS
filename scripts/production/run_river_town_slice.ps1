...
if (-not $Interactive) { $sliceArgs += @('--quit-after','180','--','--capture') }
& $Godot @sliceArgs
...
if (-not $Interactive -and (Get-Item -LiteralPath (Join-Path $sliceEvidence 'river_town_actual_1920x1080.png')).LastWriteTimeUtc -lt $captureStarted) { throw 'Runtime did not produce a fresh screenshot. Previous evidence was preserved.' }
