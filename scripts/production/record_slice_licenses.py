...
def digest(path,kind='sha256'):
    ...
    return h.hexdigest()
names=['fir_sapling_medium','island_tree_01','shrub_02','rock_moss_set_01','barrel_03','wooden_military_crate','worn_plaster_wall','roof_tiles_14','kloofendal_48d_partly_cloudy_puresky']
rows=[]
for name in names:
    ...
    if name not in ['worn_plaster_wall','roof_tiles_14'] and 'gltf' in data:
        ...
        row['modifications']='Blender GLB conversion, bounded decimation, original PBR textures retained. Complete plant and rock variants re-centered separately. Original multi-part crate stays assembled. Far foliage cards rendered from these meshes in Godot 4.7.1; these are runtime assets, not final-frame replacements.'
    elif 'hdri' in data:
        ...
        row['modifications']='Unchanged HDR pixels; Godot sky rotation and intensity are scene lighting parameters.'
    else:
...
with urllib.request.urlopen(urllib.request.Request('https://polyhaven.com/license',headers={'User-Agent':'FrontlineVisualResearch/1.0'}),timeout=25) as response:
    (out/'polyhaven_license_20260908.html').write_bytes(response.read())
