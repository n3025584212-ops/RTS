...
func verify() -> void:
	var scene := load("res://scenes/production/RiverTownHeroHouseStructureStudy.tscn") as PackedScene
	...
	for index in range(instances.size()):
		...
		if meshes.size() != 9:failures.append("House %d changed authored mesh batching" % index)
	...
	var report := {"instances":3,"placements":"translated, rotated, uniformly scaled, then moved after ready","mesh_resources_shared":true,"surfaces_per_house":9,"failures":failures,"passed":failures.is_empty()}
	...
	FileAccess.open("res://artifacts/visual_reset/batch02_replication_validation.json",FileAccess.WRITE).store_string(JSON.stringify(report,"\t"))
